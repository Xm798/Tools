// https://gitee.com/thewindsword/MyMobileGameTools/blob/dev/Genshin%20Impact/Scriptable/GenshinInfo.js


const config = new Array();
// ========= ↓将生成的配置粘贴这以下↓=========
config[0] = "游戏的uid"
//天空岛：cn_gf01  世界树：cn_qd01  美服：os_usa  欧服：os_euro  亚服：os_asia  台港澳服：os_cht
config[1] = "os_asia"
config[2] = "Hoyolab的Cookie"
// ========= ↑将生成的配置粘贴这以上↑ ========
/**
...
async function createWidget() {
  ...
  gradient.colors = [new Color("141414"), new Color("2a5398")];
  ...
  let ResinElement = resinStack.addText(
    `当前树脂：${resin.current_resin} / ${resin.max_resin}`
  );
  ...
  let CoinElement = coinStack.addText(
    `当前宝钱：${resin.current_home_coin} / ${resin.max_home_coin}`
  );
  ...
  let ResinDiscountTextElement = resinDiscountStack.addText(
    `每周树脂减半次数：${resin.remain_resin_discount_num} / ${resin.resin_discount_num_limit}`
  );
  ...
  let ResinDiscountTipElement = resinDiscountTipStack.addText(
    `${await getTime(recoverWeekTime)}(${await getClock(recoverWeekTime)})`
  );
  ...
  let TaskElement = taskStack.addText(
    `每日委托：${resin.finished_task_num} / ${resin.total_task_num}`
  );
  ...
  let TaskTipElement = taskTipStack.addText(
    `${await getTime(recoverTaskTime)}(${await getClock(recoverTaskTime)})`
  );
  ...
  for (let i = -1; i++ < resin.max_expedition_num;) {
    ...
    if (isOngoing) {
      ...
      if (expeditions[i].status !== "Finished") {
        let remainedTimeElemnet = expeditionStack.addText(
          `${await getClock(remained_time)}`
        );
  ...
  if (isHasFinished) {
    let expeditionsRecoverElement =
      expeditionsTitleStack.addText(`已有派遣完成`);
    ...
    let expeditionsRecoverElement = expeditionsTitleStack.addText(
      `最早完成时间：${await getClock(minCoverTime)}`
    );
...
async function getData() {
  ...
  let sign = md5("salt=okr4obncj8bw5a65hbnn5oo6ixjc3l9w&t=" + timestamp + "&r=" + randomStr + "&b=&q=role_id=" + config[0] + "&server=" + config[1])

  let req = new Request("https://bbs-api-os.mihoyo.com/game_record/app/genshin/api/dailyNote?server=" + config[1] + "&role_id=" + config[0])
  req.method = "get"
  req.headers = {
