...
const widgetSizer = {
  ...
  async function createWidget() {
    ...
    if (isSmall) {
      ...
      resinText = "树脂："
      resinTipText = `${await getTime(resin.resin_recovery_time)}(${await getClock(resin.resin_recovery_time)})`
    }
    let ResinElement = resinStack.addText(resinText + `${resin.current_resin} / ${resin.max_resin}`)
  ...
  //   coinStack.addSpacer(4)  
    ...
    if (isSmall) {
      ...
      coinText = "宝钱："
      coinTipText = `${await getTime(resin.home_coin_recovery_time)}(${await getClock(resin.home_coin_recovery_time)})`
    }
    let CoinElement = coinStack.addText(coinText + `${resin.current_home_coin} / ${resin.max_home_coin}`)
  ...
  //   coinStack.addSpacer(10)
    ...
    let ResinDiscountTipElement = resinDiscountTipStack.addText(resinDiscountTipText)
    ...
    if (isSmall) {
      ...
      taskText = "每日委托："
      taskTipText = `${await getTime(recoverTaskTime)}(${await getClock(recoverTaskTime)})`
    }
    let TaskElement = taskStack.addText(taskText + `${resin.finished_task_num} / ${resin.total_task_num}`)
  ...
  //     let expeditionsStack = i > 2 ? expeditionsStackTwo : expeditionsStackOne
    ...
    if (isHasFinished) {
      let expeditionsRecoverElement = expeditionsTitleStack.addText(isSmall ? "":`已有派遣完成`)
      ...
      let expeditionsRecoverElement = expeditionsTitleStack.addText(isSmall ? "":`最早完成时间：${await getClock(minCoverTime)}`)
