// https://gitee.com/ugna/tools4games/blob/master/Genshin/GenshinInfo_Large.js
...
let isMedium = config.widgetFamily == "medium";
if (isSmall) {
  ThemeConfig = widgetSizer.small;
} else if (isMedium) {
  ThemeConfig = widgetSizer.medium;
} else {
  ThemeConfig = widgetSizer.large;
}
...
async function createWidget() {
  ...
  // 自定义背景渐变颜色
  gradient.colors = [new Color("a2ddb5"), new Color("2a5398")];
  ...
  let resinTipText = "";
  if (isSmall) {
    resinTipText = `${await getClock(resin.resin_recovery_time)}`;
  } else {
    resinText = "树脂：";
    resinTipText = `${await getTime(
      resin.resin_recovery_time
    )}(${await getClock(resin.resin_recovery_time)})`;
  }
  let ResinElement = resinStack.addText(
    resinText + `${resin.current_resin} / ${resin.max_resin}`
  );
  ResinElement.textColor = Color.white();
  ResinElement.textOpacity = 1;
  ResinElement.font = Font.mediumSystemFont(ThemeConfig.textSize);

  let ResinTipElement = resinTipStack.addText(resinTipText);
  ...
  // 宝钱
  let coinStack = bottomRightStack.addStack();
  let coinTipStack = bottomRightStack.addStack();
  ...
  coinStack.addSpacer(ThemeConfig.iconSpacer);
  //   coinStack.addSpacer(4)
  let coinText = "";
  let coinTipText = "";
  if (isSmall) {
    coinTipText = `${await getClock(resin.home_coin_recovery_time)}`;
  } else {
    coinText = "宝钱：";
    coinTipText = `${await getTime(
      resin.home_coin_recovery_time
    )}(${await getClock(resin.home_coin_recovery_time)})`;
  }
  let CoinElement = coinStack.addText(
    coinText + `${resin.current_home_coin} / ${resin.max_home_coin}`
  );
  CoinElement.textColor = Color.white();
  CoinElement.textOpacity = 1;
  CoinElement.font = Font.mediumSystemFont(ThemeConfig.textSize);
  //   coinStack.addSpacer(10)
  let CoinTipElement = coinTipStack.addText(coinTipText);
  ...
  let resinDiscountTipStack = topRightStack.addStack();
  ...
  let resinDiscountText = "";
  let resinDiscountTipText = "";
  let recoverWeekTime = (weekEnd - new Date()) / 1000;
  if (isSmall) {
    resinDiscountTipText = `${await getClock(recoverWeekTime)}`;
  } else {
    resinDiscountText = "每周树脂减半：";
    resinDiscountTipText = `${await getTime(recoverWeekTime)}(${await getClock(
      recoverWeekTime
    )})`;
  }
  let ResinDiscountTextElement = resinDiscountStack.addText(
    resinDiscountText +
      `${resin.remain_resin_discount_num} / ${resin.resin_discount_num_limit}`
  );
  ...
  let ResinDiscountTipElement =
    resinDiscountTipStack.addText(resinDiscountTipText);
  ResinDiscountTipElement.textColor = Color.white();
  ResinDiscountTipElement.textOpacity = 0.6;
  ResinDiscountTipElement.font = Font.mediumRoundedSystemFont(
    ThemeConfig.tipSize
  );
  resinDiscountStack.centerAlignContent();

  let taskStack = bottomLeftStack.addStack();
  let taskTipStack = bottomLeftStack.addStack();
  ...
  if (isSmall) {
    taskTipText = `${await getClock(recoverTaskTime)}`;
  } else {
    taskText = "每日委托：";
    taskTipText = `${await getTime(recoverTaskTime)}(${await getClock(
      recoverTaskTime
    )})`;
  }
  let TaskElement = taskStack.addText(
    taskText + `${resin.finished_task_num} / ${resin.total_task_num}`
  );
  TaskElement.textColor = Color.white();
  TaskElement.textOpacity = 1;
  TaskElement.font = Font.mediumSystemFont(ThemeConfig.textSize);

  let TaskTipElement = taskTipStack.addText(taskTipText);
  ...
  let minCoverTime = 0;
  let expeditionsTitleElement = expeditionsTitleStack.addText(
    `每日派遣：${resin.current_expedition_num} / ${resin.max_expedition_num}`
  );
  ...
  /* let expeditionsStackTwo = widget.addStack()
    expeditionsStackTwo.layoutVertically()
  ...
  for (let i = -1; i++ < resin.max_expedition_num; ) {
    ...
    if (isOngoing) {
      ...
      let remainedTimeText = `${await getClock(remained_time)}`;
      let finishedText = " 探索完成";
      if (isSmall) {
        remainedTimeText = remainedTimeText.slice(5);
        log(remainedTimeText);
        finishedText = " 完成";
      }
      if (expeditions[i].status !== "Finished") {
        let remainedTimeElemnet = expeditionStack.addText(remainedTimeText);
        ...
        isHasFinished = true;
        let remainedTimeElemnet = expeditionStack.addText(finishedText);
        remainedTimeElemnet.rightAlignText();
  ...
  if (isHasFinished) {
    let expeditionsRecoverElement = expeditionsTitleStack.addText(
      isSmall ? "" : `已有派遣完成`
    );
    expeditionsRecoverElement.textColor = Color.green();
    expeditionsRecoverElement.textOpacity = 0.8;
    expeditionsRecoverElement.font = Font.mediumSystemFont(
      ThemeConfig.textSize
    );
  } else {
    let expeditionsRecoverElement = expeditionsTitleStack.addText(
      isSmall ? "" : `最早完成时间：${await getClock(minCoverTime)}`
    );
...
async function getTime(time) {
  let hh = ~~(time / 3600);
  let mm = ~~((time % 3600) / 60);
  let hfm = hh < 10 ? "0" + hh : hh;
  let mfm = mm < 10 ? "0" + mm : mm;
  return "- " + hfm + ":" + mfm + " ";
}
...
async function getClock(time) {
  ...
  let hh = timeRecovery.getHours();
  let mm = timeRecovery.getMinutes();
  let hfm = hh < 10 ? "0" + hh : hh;
  let mfm = mm < 10 ? "0" + mm : mm;
